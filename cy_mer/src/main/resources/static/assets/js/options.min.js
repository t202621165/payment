(function($) {
	...
	$._option = function(option) {
		...
		var optional = avalon.define({
			...
			currency: function(e) {
				$("#currencyName").val(e.target.value);
				$("#scriptCommand").val($(e.target).attr("data-command"));
			},
			...
			validate: {
				onValidateAll: function(reasons) {
					if (reasons.length) {
	                	...
	                	if (option.url) {
	                		option.pre && option.pre();
	                		
	                		$.ajax({
	                			url: option.url,
	                			data: $(this).serialize(),
	                			success: function(data) {
	                				!option.hideMsg && data.msg && $.modalmsg(data.msg);
	                				
	                				option.post && option.post(data);
	                				if (data.ele) {
	                					$("[name="+data.ele+"]").focus();
	                				}
	                				...
	                				if (data.state && optional.data.callback)
	                					optional.data.callback(data.msg);
	...
	$._merge = function(options) {
		options && $.modalbox({
			...
			yes: function(data) {
				...
				(result || !options.url) && options.render && options.render();
				return (result || !options.url);
			}
		});
	};
